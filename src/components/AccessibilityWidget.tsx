...
function savePosition(x: number, y: number, isCustom: boolean) {
  try {
    localStorage.setItem(ACC_POS_KEY, JSON.stringify({
      x,
      y,
      isCustom
    }));
...
const AccessibilityWidget: React.FC = () => {
  ...
  const texts = useMemo(() => ({
    he: {
      ...
      shortcuts: "Alt+Shift+A לפתיחה · C ניגודיות · I היפוך · U קישורים · G אפור · M שחור־לבן · H כותרות · L תמונות · X אנימציות · R איפוס · +/- גודל · 0 איפוס גודל · Esc סגור",
      label: "כלי נגישות",
    ...
    fr: {
      ...
      shortcuts: "Alt+Shift+A ouvrir · C contraste · I inverser · U liens · G gris · M mono · H titres · L images · X anim · R reset · +/- taille · 0 reset taille · Esc fermer",
    ...
    en: {
      ...
      shortcuts: "Alt+Shift+A open · C contrast · I invert · U links · G gray · M mono · H headings · L images · X anim off · R reset · +/- size · 0 reset size · Esc close",
      label: "Accessibility tools",
      speakSel: "Speak selection",
      stop: "Stop speech"
    }
  }), []);
  const l = (texts as any)[language as keyof typeof texts ?? "en"] ?? (texts as any).en;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (k === "h") setPrefs(p => ({
        ...p,
        highlightH: !p.highlightH
      }));
      if (k === "l") setPrefs(p => ({
        ...p,
        hideImages: !p.hideImages
      }));
      if (k === "r") handleReset();
      if (k === "=" || k === "+") setPrefs(p => ({
        ...p,
        fontPercent: Math.min(160, p.fontPercent + 10)
      }));
      if (k === "-") setPrefs(p => ({
        ...p,
        fontPercent: Math.max(80, p.fontPercent - 10)
      }));
      if (k === "0") setPrefs(p => ({
        ...p,
        fontPercent: 100
  ...
  useEffect(() => {
    const anyCustom = JSON.stringify(prefs) !== JSON.stringify(defaultPrefs);
  ...
  const Panel = <div role="dialog" aria-label={l.title} aria-modal="false" className="acc-panel" style={{
    position: "fixed",
    zIndex: 2147483646,
      ...
      maxHeight: "70vh"
        ...
        padding: "18px 20px"
          ...
          gap: 8
            ...
            {[["grayscale", l.grayscale], ["contrast", l.contrast], ["invert", l.invert], ["monochrome", l.monochrome], ["underline", l.underline], ["hideImages", l.hideImages], ["dyslexia", l.dyslexia], ["readable", l.readable], ["guide", l.guide], ["highlightH", l.highlightH], ["cursorLight", l.cursorLight], ["cursorDark", l.cursorDark], ["noAnim", l.noAnim]].map(([key, label]) => {
            const active = (prefs as any)[key];
            return <button key={key} type="button" aria-pressed={!!active} onClick={() => setPrefs(p => ({
              ...p,
              [key]: !active
            }) as any)} style={{
              textAlign: "center",
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button type="button" onClick={() => setPrefs(p => ({
              ...p,
              fontPercent: Math.max(60, p.fontPercent - 10)
            }))} style={{
              background: "#ffffff",
              border: "none",
              fontSize: 28,
            ...
            <button type="button" onClick={() => setPrefs(p => ({
              ...p,
              fontPercent: Math.min(250, p.fontPercent + 10)
            }))} style={{
              background: "#ffffff",
              border: "none",
              fontSize: 28,
          ...
          <input aria-label={l.fontSize} type="range" min={60} max={250} value={prefs.fontPercent} onChange={e => setPrefs(p => ({
            ...p,
            fontPercent: parseInt(e.target.value, 10)
          }))} style={{
            width: "100%",
            accentColor: "#0BA5E9",
            marginTop: 12
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button type="button" onClick={() => setPrefs(p => ({
              ...p,
              wordSpacing: Math.max(0, p.wordSpacing - 1)
            }))} style={{
              background: "#ffffff",
              border: "none",
              fontSize: 28,
            ...
            <button type="button" onClick={() => setPrefs(p => ({
              ...p,
              wordSpacing: Math.min(16, p.wordSpacing + 1)
            }))} style={{
              background: "#ffffff",
              border: "none",
              fontSize: 28,
          ...
          <input aria-label={l.wordSpacing} type="range" min={0} max={16} step={0.1} value={prefs.wordSpacing} onChange={e => setPrefs(p => ({
            ...p,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button type="button" onClick={() => setPrefs(p => ({
              ...p,
              letterSpacing: Math.max(0, p.letterSpacing - 0.1)
            }))} style={{
              background: "#ffffff",
              border: "none",
              fontSize: 28,
            ...
            <button type="button" onClick={() => setPrefs(p => ({
              ...p,
              letterSpacing: Math.min(8, p.letterSpacing + 0.1)
            }))} style={{
              background: "#ffffff",
              border: "none",
              fontSize: 28,
          ...
          <input aria-label={l.letterSpacing} type="range" min={0} max={8} step={0.01} value={prefs.letterSpacing} onChange={e => setPrefs(p => ({
            ...p,
        ...
        {canSpeak && <div style={{
          ...
          gap: 8
            ...
            <button type="button" onClick={speakSelection} disabled={isSpeaking} style={{
            flex: 1,
            padding: "10px",
            borderRadius: 8,
            ...
            <button type="button" onClick={stopSpeak} disabled={!isSpeaking} style={{
            flex: 1,
            padding: "10px",
            borderRadius: 8,
  ...
  return <>
      ...
      {/* Skip link */}
      <a href="#main-content" className="acc-skip-link">
        {l.skip}
      </a>
